...
@Composable
fun ButtonThemeSwitcher(
    modifier: Modifier = Modifier,
    isDarkMode: MutableState<Boolean>, //todo переделать на тему
) {
    val dampingRatio by remember { mutableStateOf(0.5f) }
    val stiffness by remember { mutableStateOf(100f) }
    ...
    Box(
        ...
        val sunMoonState = when (isDarkMode.value) {
            false -> SunMoonState.Sun
            true -> SunMoonState.Moon
        }
        SunMoonIcon(
            sunMoonState,
            springSpec = realSpringSpec,
            fillColor = if (isDarkMode.value) Color(0xFFe5e5e5) else Color(0xFFFAC668)
        )
    }
}
