...
class LocalBookInfoDataSource(
    ...
    suspend fun addOrUpdateBooks(books: List<BookEntity>, userId: Int) {
        books.forEach { book ->
    ...
    suspend fun getNotSynchronizedBooks(userId: Int): List<BookEntity> {
    ...
    suspend fun getLocalBookById(bookId: String, userId: Int): Flow<List<BookEntity>> =
        booksDao.getLocalBookById(bookId = bookId, userId = userId)

    suspend fun updateBookAndTime(book: BookEntity, userId: Int): BookEntity {
